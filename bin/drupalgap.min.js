...
function drupalgap_onload(){try{for(var a="drupalgap block comment contact entity field file image menu mvc node search system taxonomy user views".split(" "),b=0;b<a.length;b++){var c=a[b];Drupal.modules.core[c]=module_object_template(c)}document.addEventListener("deviceready",_drupalgap_deviceready,!1)}catch(d){console.log("drupalgap_onload - "+d)}}
...
function drupalgap_alert(a,b){try{var c=null;b&&(c=b);var d=function(){},f="Alert",e="OK";c&&(c.alertCallback&&(d=c.alertCallback),c.title&&(f=c.title),c.buttonName&&(e=c.buttonName));"undefined"===typeof navigator.notification?(alert(a),d()):navigator.notification.alert(a,d,f,e)}catch(g){console.log("drupalgap_alert - "+g)}}
...
function drupalgap_check_connection(){try{if("function"===typeof parent.window.ripple)return drupalgap.online=!0,"Ethernet connection";var a=navigator.connection.type,b={};b[Connection.UNKNOWN]="Unknown connection";b[Connection.ETHERNET]="Ethernet connection";b[Connection.WIFI]="WiFi connection";b[Connection.CELL_2G]="Cell 2G connection";b[Connection.CELL_3G]="Cell 3G connection";b[Connection.CELL_4G]="Cell 4G connection";b[Connection.NONE]="No network connection";drupalgap.online="No network connection"==
b[a]?!1:!0;return b[a]}catch(c){console.log("drupalgap_check_connection - "+c)}}function drupalgap_empty(a){try{return console.log("WARNING: drupalgap_empty() is deprecated! Use empty() instead."),empty(a)}catch(b){console.log("drupalgap_empty - "+b)}}function drupalgap_file_exists(a){try{var b=!1;jQuery.ajax({async:!1,type:"HEAD",url:a,success:function(){b=!0},error:function(a,b,c){}});return b}catch(c){console.log("drupalgap_file_exists - "+c)}}
...
a+")");return c}catch(f){console.log("drupalgap_get_path - "+f)}}function drupalgap_back(){try{$(".ui-page-active").attr("id")==drupalgap.settings.front?drupalgap_confirm("Exit "+drupalgap.settings.title+"?",{confirmCallback:_drupalgap_back_exit}):_drupalgap_back()}catch(a){console.log("drupalgap_back"+a)}}function _drupalgap_back(){try{drupalgap.back=!0,history.back(),drupalgap_path_set(drupalgap.back_path)}catch(a){console.log("drupalgap_back"+a)}}
function _drupalgap_back_exit(){try{navigator.app.exitApp()}catch(a){console.log("_drupalgap_back_exit - "+a)}}function drupalgap_error(a){try{var b="drupalgap_error() - "+arguments.callee.caller.name+" - "+a;dpm(b);Drupal.settings.debug&&drupalgap_alert(b);arguments[1]&&drupalgap_alert(arguments[1]);"error"!=drupalgap_path_get()&&drupalgap_goto("error")}catch(c){console.log("drupalgap_error - "+c)}}
...
">"+k.replace(/ui-btn-right/g,"")+"</div>");c+=h+k}var v=0,x=0;$.each(drupalgap.settings.blocks[drupalgap.settings.theme][a.name],function(d,e){var f=!1;drupalgap_check_visibility("block",e)&&(f=!0,"main"==d||"offline"!=b&&"error"!=b||(f=!1));f&&(f=drupalgap_block_load(d),v++,menu_load(d)&&x++,f&&(c+=module_invoke(f.module,"block_view",d,a)))});if(in_array(a.attributes["data-role"],["header","footer"])&&(0==v&&0<d||0==v-x)||0<d&&f>=x&&0==v)c+="<h2>&nbsp;</h2>";c+="</div>\x3c!-- "+a.name+" --\x3e"}return c}catch(y){console.log("drupalgap_render_region - "+
y)}}
function _drupalgap_page_title_pageshow(a){try{var b=drupalgap_router_path_get(),c=[];"undefined"!==typeof drupalgap.menu_links[b].title&&drupalgap_set_title(drupalgap.menu_links[b].title);if("undefined"!==typeof drupalgap.menu_links[b].title_callback){var d=drupalgap.menu_links[b].title_callback;if(drupalgap_function_exists(d)){var f=window[d];c.unshift(_drupalgap_page_title_pageshow_success);if(drupalgap.menu_links[b].title_arguments){var e=arg(null,drupalgap_path_get());$.each(drupalgap.menu_links[b].title_arguments,function(a,
b){is_int(b)&&e[b]?c.push(e[b]):c.push(b)})}drupalgap_set_title(f.apply(null,Array.prototype.slice.call(c)))}}else _drupalgap_page_title_pageshow_success(drupalgap_get_title())}catch(g){console.log("_drupalgap_page_title_pageshow - "+g)}}function _drupalgap_page_title_pageshow_success(a){try{var b=system_title_block_id(drupalgap_path_get());$("h1#"+b).html(a)}catch(c){console.log("_drupalgap_page_title_pageshow_success - "+c)}}
...
function theme_email(a){try{return a.attributes.type="email","<input "+drupalgap_attributes(a.attributes)+" />"}catch(b){console.log("theme_email - "+b)}}
...
function theme_search(a){try{return a.attributes.type="search","<input "+drupalgap_attributes(a.attributes)+" />"}catch(b){console.log("theme_search - "+b)}}function theme_select(a){try{var b="";a.options&&$.each(a.options,function(c,f){if("attributes"!=c){var e="";"undefined"!==typeof a.value&&a.value==c&&(e=" selected ");b+='<option value="'+c+'" '+e+">"+f+"</option>"}});return"<select "+drupalgap_attributes(a.attributes)+">"+b+"</select>"}catch(c){console.log("theme_select - "+c)}}
...
window[d].call(null,b)}catch(e){console.log("theme - "+e)}}var _theme_autocomplete_input_selector,_theme_autocomplete_remote,_theme_autocomplete_variables;
function theme_autocomplete(a){try{var b="";_theme_autocomplete_variables=a;var c=!1;a.remote&&(c=!0);a.remote=c;_theme_autocomplete_remote=a.remote;c=null;a.attributes.id?c=a.attributes.id:(c="autocomplete_"+user_password(),a.attributes.id=c);var b=b+theme("hidden",{attributes:{id:c}}),c=c+"-list",d={attributes:{id:c,"data-role":"listview","data-filter":"true","data-inset":"true","data-filter-placeholder":"..."}},f="";a.remote?(d.items=[],f+='<script type="text/javascript">$("#'+c+'").on("filterablebeforefilter", function(e, d) { _theme_autocomplete(this, e, d); });\x3c/script>'):
(d.items=_theme_autocomplete_prepare_items(a),d.attributes["data-filter-reveal"]=!0);var e="#"+drupalgap_get_page_id()+' input[data-type="search"]',f=f+('<script type="text/javascript">_theme_autocomplete_input_selector = \''+e+"';\x3c/script>"),b=b+theme("item_list",d);return b+f}catch(g){console.log("theme_autocomplete - "+g)}}
function _theme_autocomplete(a,b,c){try{if("undefined"===typeof _theme_autocomplete_variables.filter)console.log('_theme_autocomplete - A "filter" was not supplied.');else{var d="undefined"!==typeof _theme_autocomplete_variables.value?!0:!1,f="undefined"!==typeof _theme_autocomplete_variables.label?!0:!1;if(d||f){d?f||(_theme_autocomplete_variables.label=_theme_autocomplete_variables.value):_theme_autocomplete_variables.value=_theme_autocomplete_variables.label;var e=$(a),g=$(c.input).val(),h="";
e.html("");if(g&&0<g.length){e.html('<li><div class="ui-loader"><span class="ui-icon ui-icon-loading"></span></div></li>');e.listview("refresh");var k=_theme_autocomplete_variables.path+"?"+_theme_autocomplete_variables.filter+"="+encodeURIComponent(g);_theme_autocomplete_variables.params&&(k+="&"+_theme_autocomplete_variables.params);views_datasource_get_view_result(k,{success:function(a){var b=!0;_theme_autocomplete_variables.custom&&(b=!1);var c=null,c=b?a[a.view.root]:a;if(0!=c.length){var d=
[],f=_theme_autocomplete_variables.value,g=_theme_autocomplete_variables.label;$.each(c,function(c,e){var h=null,h=b?e[a.view.child]:e;d.push({value:h[f],label:h[g]})});0!=d.length&&(_theme_autocomplete_variables.items=d,c=_theme_autocomplete_prepare_items(_theme_autocomplete_variables),$.each(c,function(a,b){h+="<li>"+b+"</li>"}),e.html(h),e.listview("refresh"),e.trigger("updatelayout"))}}})}}else console.log('_theme_autocomplete - A "value" and/or "label" was not supplied.')}}catch(m){console.log("_theme_autocomplete - "+
m)}}function _theme_autocomplete_prepare_items(a){try{var b=[];a.items&&(b=a.items);var c=[];0<b.length&&$.each(b,function(b,d){var g="",h="";"string"===typeof d?h=g=d:(g=d.value,h=d.label);g=l(h,null,{attributes:{value:g,onclick:"_theme_autocomplete_click('"+a.attributes.id+"', this)"}});c.push(g)});return c}catch(d){console.log("_theme_autocomplete_prepare_items - "+d)}}
function _theme_autocomplete_click(a,b){try{var c=a+"-list";$("#"+a).val($(b).attr("value"));$(_theme_autocomplete_input_selector).val($(b).html());_theme_autocomplete_remote?$("#"+c).html(""):($("#"+c+" li").addClass("ui-screen-hidden"),$("#"+c).listview("refresh"));if(_theme_autocomplete_variables.item_onclick&&drupalgap_function_exists(_theme_autocomplete_variables.item_onclick))(0,window[_theme_autocomplete_variables.item_onclick])(a,$(b))}catch(d){console.log("_theme_autocomplete_click - "+d)}}
function theme_button(a){try{return a.attributes["data-role"]="button","<a "+drupalgap_attributes(a.attributes)+">"+a.text+"</a>"}catch(b){console.log("theme_button_link - "+b)}}function theme_button_link(a){try{return a.attributes["data-role"]="button",theme_link(a)}catch(b){console.log("theme_button_link - "+b)}}
...
typeof a.attributes.href&&(a.attributes.href="#");return"<a "+drupalgap_attributes(a.attributes)+">"+b+"</a>"}catch(f){console.log("theme_link - "+f)}}function theme_popup(a){try{a.attributes["data-role"]="popup";var b={};a.button_attributes&&(b=a.button_attributes);b.href="#"+a.attributes.id;b["data-rel"]="popup";return bl(a.button_text,null,{attributes:b})+"<div "+drupalgap_attributes(a.attributes)+">"+a.content+"</div>"}catch(c){console.log("theme_popup - "+c)}}
...
function template_process_page(a){try{var b=drupalgap_path_get();drupalgap.output=menu_execute_active_handler();var c=drupalgap_get_page_id(b);$.each(drupalgap.theme.regions,function(a,b){var d=$("#"+c).html(),d=d.replace("{:"+b.name+":}",drupalgap_render_region(b));$("#"+c).html(d)})}catch(d){console.log("template_process_page - "+d)}}
...
d)}}
function drupalgap_entity_render_content(a,b){try{b.content="";var c=b.type;"comment"==a&&(c=b.bundle);var d=drupalgap_field_info_instances(a,c);if(d){var f={},e={};$.each(d,function(c,d){if(!d.display)return!1;var g=d.display["default"];d.display.drupalgap&&(g=d.display.drupalgap,"undefined"===typeof g.module&&"undefined"!==typeof d.display["default"].module&&(g.module=d.display["default"].module));"hidden"!=g.type&&(e[c]=g.weight,f[c]=drupalgap_entity_render_field(a,b,c,d,g))});var g=[];$.each(e,function(a,
b){g.push(b)});g.sort(function(a,b){return a-b});var h=[];$.each(g,function(a,c){$.each(e,function(a,d){if(c==d&&-1==h.indexOf(a))return h.push(a),b.content+=f[a],!1})});module_invoke_all("entity_post_render_content",b,a,c);_entity_local_storage_save(a,b[entity_primary_key(a)],b)}}catch(k){console.log("drupalgap_entity_render_content - "+k)}}
function drupalgap_entity_render_field(a,b,c,d,f){try{var e="",g=f.module;if(!g)if(d.widget.module)g=d.widget.module;else return console.log("drupalgap_entity_render_field - unable to locate the module for the field ("+c+")"),e;g+="_field_formatter_view";if(drupalgap_function_exists(g)){var h=window[g],g=null,k=b.language;b[c]&&(b[c][k]?g=b[c][k]:b[c].und?(g=b[c].und,k="und"):g=b[c]);var m=h(a,b,d,drupalgap_field_info_field(c),k,g,f);$.each(m,function(a,b){var c="";b.markup?c=b.markup:b.theme&&(c=
theme(b.theme,b));e+=c})}else console.log("WARNING: drupalgap_entity_render_field - "+g+"() does not exist! ("+c+")");if(""!=e&&"hidden"!=f.label){var n="<h3>"+d.label+"</h3>",n="<div>"+n+"</div>";switch(f.label){case "below":e+=n;break;default:e=n+e}}e='<div class="'+c+'">'+e+"</div>";a={content:e};module_invoke_all("entity_post_render_field",b,c,d,a);return a.content!=e?a.content:e}catch(p){console.log("drupalgap_entity_render_field - "+p)}}
function drupalgap_entity_build_from_form_state(a,b){try{var c={},d=language_default();$.each(b.values,function(b,f){var h=!1,k=!1,k=!1;a.elements[b].is_field&&(h=!0,k=a.elements[b].field_info_field.module,k+="_assemble_form_state_into_field",function_exists(k)||(k=!1));var m=drupalgap_field_key(b);if(m){var n=a.elements[b].field_info_field.cardinality;-1==n&&(n=1);if("undefined"!==typeof f[d][0]){c[b]={};var p=!0;"taxonomy_term_reference"==a.elements[b].type||"options_select"==a.elements[b].field_info_instance.widget.type?
(p=!1,c[b][d]={}):c[b][d]=[];for(var q=0;q<n;q++)if("undefined"!==typeof f[d][q]){var r=f[d][q],s={value:"value",use_key:!0,use_wrapper:!0,use_delta:p};h&&k&&(r=(0,window[k])(a.entity_type,a.bundle,r,a.elements[b].field_info_field,a.elements[b].field_info_instance,d,q,s));m!=s.value&&(m=s.value);s.use_delta?s.use_key?(s={},s[m]=r,c[b][d].push(s)):c[b][d].push(r):s.use_wrapper?$.isArray(c[b][d])?(console.log("WARNING: drupalgap_entity_build_from_form_state - cannot use key ("+m+") on field ("+b+") language code array, key will be ignored."),
c[b][d].push(r)):c[b][d][m]=r:c[b][d]=r;null===r&&"undefined"!==typeof c[b]&&"list_boolean"!=a.elements[b].type&&delete c[b];!h||p||"options_select"!=a.elements[b].field_info_instance.widget.type||a.elements[b].required||""!==r||"undefined"===typeof c[b]||delete c[b]}}}else"undefined"!==typeof f&&(c[b]=f)});return c}catch(f){console.log("drupalgap_entity_build_from_form_state - "+f)}}
...
f)}},error:function(c,d,e){try{var f=_drupalgap_form_submit_response_errors(a,b,c,d,e);f&&drupalgap_alert(f)}catch(g){console.log("drupalgap_entity_form_submit - error - "+g)}}})}catch(g){console.log("drupalgap_entity_form_submit - "+g)}}
...
function node_page_view_pageshow(a){try{node_load(a,{success:function(a){var b={theme:"node",node:a,title:{markup:a.title},content:{markup:a.content}};if(0!=a.comment){if(1==a.comment||2==a.comment){var f="";2==a.comment&&(f=drupalgap_get_form("comment_edit",{nid:a.nid},a));0!=a.comment_count?comment_index({parameters:{nid:a.nid}},{success:function(e){try{var g="";$.each(e,function(a,b){g+=theme("comment",{comment:b})});b.content.markup+=theme("comments",{node:a,comments:g});2==a.comment&&(b.content.markup+=
f);_drupalgap_entity_page_container_inject("node",a.nid,"view",b)}catch(h){console.log("node_page_view_pageshow - comment_index - "+h)}}}):(2==a.comment&&(b.content.markup+=theme("comments",{node:a}),b.content.markup+=f),_drupalgap_entity_page_container_inject("node",a.nid,"view",b))}}else b.content.markup+=theme("comments",{node:a}),_drupalgap_entity_page_container_inject("node",a.nid,"view",b)}})}catch(b){console.log("node_page_view_pageshow - "+b)}}
...
function system_dashboard_page(){try{var a={};a.site_info={markup:'<h4 style="text-align: center;">'+Drupal.settings.site_path+"</h4>"};a.welcome={markup:'<h2 style="text-align: center;">Welcome to DrupalGap</h2><p>The open source mobile application development kit for Drupal!</p>'};drupalgap.settings.logo&&(a.logo={markup:"<center>"+theme("image",{path:drupalgap.settings.logo})+"</center>"});a.get_started={theme:"button_link",text:"Getting Started Guide",path:"http://www.drupalgap.org/get-started",
...
function _taxonomy_field_widget_form_autocomplete(a,b,c,d,f){try{var e=$(c),g=$(f.input),h=g.val(),k="";_taxonomy_field_widget_form_autocomplete_input=g;$(_taxonomy_field_widget_form_autocomplete_input).on("change",function(){$("#"+a).val($(this).val())});e.html("");if(h&&0<h.length){e.html('<li><div class="ui-loader"><span class="ui-icon ui-icon-loading"></span></div></li>');e.listview("refresh");var m="drupalgap/taxonomy-term-autocomplete/"+b+"&name="+encodeURIComponent(h);views_datasource_get_view_result(m,
{success:function(c){0!=c.terms.length&&($.each(c.terms,function(c,d){var f={tid:d.term.tid,vid:b,name:d.term.name,onclick:"_taxonomy_field_widget_form_click('"+a+"', '"+e.attr("id")+"', this)"};k+="<li "+drupalgap_attributes(f)+">"+d.term.name+"</li>"}),e.html(k),e.listview("refresh"),e.trigger("updatelayout"))}})}}catch(n){console.log("_taxonomy_field_widget_form_autocomplete - "+n)}}
...
function taxonomy_views_exposed_filter(a,b,c,d,f){try{c.type="hidden",$.each(f.settings.allowed_values,function(a,b){var e={required:c.required,render_input_element:!1,attributes:{id:c.options.attributes.id},field_info_field:f,field_info_instance:{widget:{type:"options_select"}},exposed:!0};empty(d.value)||(e.value=parseInt(d.value[0]),e.attributes.value=e.value);c.children.push({markup:theme("taxonomy_term_reference",e)})})}catch(e){console.log("taxonomy_views_exposed_filter - "+e)}}
var _views_exposed_filter_query=null,_views_exposed_filter_reset=!1,_views_exposed_filter_submit_variables=null;
